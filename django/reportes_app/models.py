...
class LlamadaLogManager(models.Manager):
    ...
    def obtener_campanas_tipo_campana_campana_agente_fecha(self, tipo_campana_id=0, campana_id=0, agente_id=0, 
            incluir_sin_campana=False, phone='', call_id='', fecha_desde=None, fecha_hasta=None):
        
        queryset = self.get_queryset()
        
        # Filtro de parámetros
        if fecha_desde and fecha_hasta:
            queryset = queryset.filter(time__date__range=[fecha_desde.date(), fecha_hasta.date()])
        elif fecha_desde:
            queryset = queryset.filter(time__date__gte=fecha_desde.date())
        elif fecha_hasta:
            queryset = queryset.filter(time__date__lte=fecha_hasta.date())

        if tipo_campana_id != '0':
            queryset = queryset.filter(tipo_campana=tipo_campana_id)
        
        if campana_id != '0':
            queryset = queryset.filter(campana_id=campana_id)
        else:
            if not incluir_sin_campana:
                queryset = queryset.exclude(campana_id__in=[-1, 0])

        if agente_id != '0':
            queryset = queryset.filter(agente_id=agente_id)
            
        if phone.strip():
            queryset = queryset.filter(numero_marcado=phone.strip())
        
        if call_id.strip():
            queryset = queryset.filter(callid=call_id.strip())
            
        # Anotaciones para contar eventos
        queryset = queryset.values('campana_id').annotate(
            received=Count('id'),
            received_transferences=Count('id', filter=Q(agente_extra_id__isnull=False)),
            attended=Count('id', filter=Q(event__in=LlamadaLog.EVENTOS_FIN_CONEXION)),
            unattended=Count('id', filter=Q(event__in=LlamadaLog.EVENTOS_REJECT + list(LlamadaLog.EVENTOS_NO_CONEXION))),
            abandon=Count('id', filter=Q(event__in=LlamadaLog.EVENTOS_NO_DIALOGO)),
            expired=Count('id', filter=Q(event='EXPIRED')),
        )
        
        # Agregar porcentajes
        reporte = []
        for row in queryset:             
            # Reemplazo de match-case por if-elif-else
            if row['campana_id'] == -1:
                row['campana_nombre'] = "Llamada manual"
            elif row['campana_id'] == 0:
                row['campana_nombre'] = "Sin campaña"
            else:
                try:
                    campana_obj = Campana.objects.get(id=row['campana_id'])
                    row['campana_nombre'] = campana_obj.nombre
                except Campana.DoesNotExist:
                    row['campana_nombre'] = "Campaña desconocida"

            # Cálculo de porcentajes
            total = row['attended'] + row['unattended']
            row.update({
                'percent_attended': f"{(row['attended'] / total * 100):.2f}" if total else "0.00",
                'percent_unattended': f"{(row['unattended'] / total * 100):.2f}" if total else "0.00"
            })
            reporte.append(row)
