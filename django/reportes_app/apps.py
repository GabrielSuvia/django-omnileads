    ...
    def supervision_menu_items(self, request, permissions):
        ...
        if reportes_items:
            items.append({
                ...
                'icon': 'icon-graph',
                'id': 'menuReports',
                'children': reportes_items
            })
            
        
        # Reportes premium
        reportes_premium_items = []
        if 'reporte_premium1' in permissions:
            reportes_premium_items.append({
                'label': _('Llamadas'),
                'url': reverse('reporte_premium1'),
            })
        if 'reporte_premium_2' in permissions:
            reportes_premium_items.append({
                'label': _('Reporte 2'),
                'url': reverse('reporte_premium2'),
            })
        if reportes_premium_items:
            items.append({
                'order': 710,
                'label': _('Reportes Premium'),
                'icon': 'icon-graph',
                'id': 'menuPremiums',
                'children': reportes_premium_items
            })
    ...
    def configuraciones_de_permisos(self):
        return [
            ...
            {'nombre': 'campaign_whatsapp_report_general',
             'roles': ['Administrador', 'Gerente', 'Supervisor', 'Referente', ]},
            {'nombre': 'reporte_premium1', 'roles': ['Administrador', 'Gerente', 'Supervisor', 'Referente', ]},
            {'nombre': 'reporte_premium2', 'roles': ['Administrador', 'Gerente', 'Supervisor', 'Referente', ]},
        ]
    ...
    informacion_de_permisos = {
        ...
        'campaign_whatsapp_report_general':
            {'descripcion': _('Vista de reporte general de Whatsapp para una Campaña'),
             'version': '1.19.0'},
        'reporte_premium1':
            {'descripcion': _('Reporte premium 1'), 'version': '1.20.0'},
    }
